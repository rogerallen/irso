...
(defn ^:dynamic calc-eso [beat tonic type]
  (let [irno-seq e-1000
        ...
        intro-seq (offset-seq beat (concat-seq seq1
                                               ...
                                               (offset-seq 2 seq3)
                                               seq3))
        intro-end (num-beats intro-seq)
        foo (println "introduction from" beat "to" intro-end)

        theme-start (+ 2 (num-beats intro-seq))
        ...
        theme-end (max (num-beats theme-seq1r)
                       (num-beats theme-seq2r)
                       (num-beats theme-seq3r))
        foo (println "theme from" theme-start "to" theme-end)
        foo (println "  theme1 from" theme-start "to" (num-beats theme-seq1r))
        foo (println "  theme2 from" theme-seq2r-start "to" (num-beats theme-seq2r))
        foo (println "  theme3 from" theme-seq3r-start "to" (num-beats theme-seq3r))
        
        concl-start (+ 4 (min (num-beats theme-seq1r)
                              (num-beats theme-seq2r)
                              (num-beats theme-seq3r)))
